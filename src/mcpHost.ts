...
export function makeDevicesApi(agent: AgentClient, runs: RunController): DevicesApi {
  return {
    ...
    mapRead: () => agent.get("/map"),
    mapExplore: (input) => agent.post("/map/explore", input),
    ...
    recordPause: (recordingId, paused) => agent.post(`/record/${recordingId}/pause`, { paused }),
    runPause: (runId, paused) => agent.post(`/runs/${runId}/pause`, { paused }),
